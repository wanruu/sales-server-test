...
# 查询user1，401
query_user_1 = Step('get', '/users/{id}', '查询user1，401', path_params='{ "id": {{0.response.body.data.id}} }', expected_status_code=401)
t.add_step(query_user_1) #2

# 登录user1
login_user_1 = Step('post', '/users/login', '登录user1', body=user_1, expected_status_code=200)
login_user_1.add_post_operation(SetGlobalVariableOperation("access_token", "{{3.response.body.data.accessToken}}"))
t.add_step(login_user_1) #3

# 查询user2，401
query_user_2 = Step('get', '/users/{id}', '查询user2，401', path_params='{ "id": {{1.response.body.data.id}} }', expected_status_code=401)
t.add_step(query_user_2) #4

# 修改user2，401
user_2_update = json.dumps({"name": "Hermione", "password": "12345678"})
update_user_2 = Step('patch', '/users/{id}', '修改user2，401', path_params='{ "id": {{1.response.body.data.id}} }', body=user_2_update, expected_status_code=401)
t.add_step(update_user_2) #5

# 删除user1
delete_user_1 = Step('delete', '/users/{id}', '删除user1', path_params='{ "id": {{3.response.body.data.id}} }')
t.add_step(delete_user_1) #6

# 登录user2
login_user_2 = Step('post', '/users/login', '登录user2', body=user_2, expected_status_code=200)
login_user_2.add_post_operation(SetGlobalVariableOperation("access_token", "{{7.response.body.data.accessToken}}"))
t.add_step(login_user_2) #7

# 删除user2
delete_user_2 = Step('delete', '/users/{id}', '删除user2',path_params='{ "id": {{7.response.body.data.id}} }')
