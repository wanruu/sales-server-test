...
update_user_1 = Step('patch', '/users/{id}', '修改user1为user2的name，失败', body=user_1_update, path_params='{ "id": {{3.response.body.data.id}} }', expected_status_code=409)
...
login_user_2.add_post_operation(SetGlobalVariableOperation("access_token", "{{6.response.body.data.accessToken}}"))
t.add_step(login_user_2) #6

# 删除user2
delete_user_2 = Step('delete', '/users/{id}', '删除user2', path_params='{ "id": {{6.response.body.data.id}} }')
