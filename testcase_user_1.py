...
# 查询user，验证name与创建时一致
find_user = Step('get', '/users/{id}', '查询user，验证name与创建时一致', path_params='{ "id": {{1.response.body.data.id}} }')
find_user.add_post_operation(AssertEqualOperation('2.response.body.data.name', '{{0.request.body.name}}'))
t.add_step(find_user) #2

# 修改user，验证name与修改后一致
new_user = json.dumps({"name": "Hermione2", "password": "123456789"})
modify_user = Step('patch', '/users/{id}', '修改user，验证name与修改后一致', path_params='{ "id": {{1.response.body.data.id}} }', body=new_user)
modify_user.add_post_operation(AssertEqualOperation('3.response.body.data.name', '{{3.request.body.name}}'))
t.add_step(modify_user) #3

# 查询user，验证name已修改
find_user = Step('get', '/users/{id}', '查询user，验证name已修改', path_params='{ "id": {{1.response.body.data.id}} }')
find_user.add_post_operation(AssertEqualOperation('4.response.body.data.name', '{{3.request.body.name}}'))
